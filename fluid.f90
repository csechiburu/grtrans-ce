	  ...
	  use fluid_model_koral3dblob, only: init_koral3dblob, del_koral3dblob
...
!      real :: sigcut=1.e10
      ...
      type fluid
        ...
        real, dimension(:), allocatable :: rho,p,bmag,rho2, &
             kela,kelb,kelc,keld,Be,rhob,weights
...
!          call intiialize_mb_model(a)
    ...
    !      write(6,*) 'load'
	    ...
	    elseif(fname=='KORAL3DBLOB') then
		   call init_koral3dblob(a,ifile,.false.,fargs%dfile,fargs%hfile,fargs%nt,fargs%indf, & 
                 fargs%scalefac,nrelbin,bingammamin,bingammamax,real(fargs%r_c),real(fargs%theta_c), &
				 real(fargs%phi_c),real(fargs%n_e),real(fargs%t_e),real(fargs%b_b),real(fargs%r_b))
        endif
...
!           write(6,*) 'del_fluid: ',f%model,THINDISK
           ...
           if(f%model==SARIAF.or.f%model==POWERLAW.or.f%model==SARIAFBLOB) deallocate(f%rho2)
		   if(f%model==SARIAFBLOB.or.f%model==KORAL3DBLOB.or.f%model==BLOB) deallocate(f%rhob)
		   if(f%model==SARIAFBLOB.or.f%model==KORAL3DBLOB.or.f%model==BLOB) deallocate(f%weights)
...
!            call get_blob_fluidvars(x0,f)
 ...
 ! !      write(6,*) 'fluid: ',size(x0),f%model
      ...
      !      write(6,*) 'made it'
          ...
          CASE (KORAL3DBLOB)
             call get_koral3dblob_fluidvars(x0,real(a),f,0)
          CASE (DUMMY)
        END SELECT
        ...
        SELECT CASE(f%model)
          ...
          CASE (KORAL3DBLOB)
             call convert_fluidvars_koral3dblob(f,ncgs,ncgsnth,bcgs,tcgs,nnthcgs,sp,0)
        END SELECT
...
!        sigcut=f%sigcut
		...
		!write(*,*) 'rhocgs before nfac: ', rhocgs
		!write(*,*) 'ncgs before nfac: ', ncgs
		!write(*,*) 'bcgs before nfac: ', bcgs
! allow scaling w/ Mdot
        !write(6,*) 'convert koral nfac, sigcut, gminval: ', sp%nfac,sp%sigcut,sp%gminval
        bcgs=bcgs*sqrt(sp%nfac)
        ncgs=ncgs*sp%nfac; rhocgs=rhocgs*sp%nfac
		!write(*,*) 'rhocgs after nfac: ', rhocgs
		!write(*,*) 'ncgs after nfac: ', ncgs
		!write(*,*) 'bcgs after nfac: ', bcgs
...
!                sp%gminval,tempcgs)! en orden: rho,p,u,b,beta_trans,rlow,rhigh,tcgs (tcgs is output)
!        else if(sp%gminval.lt.0d0) then
!           tempcgs=f%p !AC the p variable stores electron temperature for KORAL
!        endif
! CE: commented lines 1161-1162 and 1169-1173 for koral3dblob (don't need them)	
        tempcgs=f%p*sp%muval	
	    ...
	    !write(*,*) 'after converting fluidvars'
	    !write(*,*) 'ncgs: ',ncgs
		!write(*,*) 'ncgsnth after convert in koral: ',ncgsnth
...
!inputs into sariaf_vals
        real, intent(in) :: a
        real, dimension(size(x0)) :: u,ctheta,gtt
...
!        not necessary to scale f%b because f%b is only used for an angle
	...
	real, dimension(size(x0)) :: n_arr, T_arr, B_arr, weights, rhob ! these are the arrays with constant n_e, T_e and B values
	...
	f%b%data(4)=0.
	!write(*,*) 'inside get_blob_fluidvars'
	f%rho=0d0
	f%bmag = B_arr
	f%rhob = n_arr
	f%p = T_arr
	...
	real, dimension(size(x0)) :: u,g00,grr,ur,gtt
	real :: n_e, t_e, b_b
	real, dimension(size(x0)) :: n_arr, T_arr, B_arr, weights, rhob
	...
	! CAVEAT: while riaf B is toroidal, here I'm rewriting it to have
	! a poloidal component instead. I did this by hand, I didn't do any thorough 
	! algebra, so this needs to be checked later
	gtt = 1.0/u**2
    f%b%data(4) = 0d0 !bmag/sqrt(bb)
    f%b%data(3) = bmag/sqrt(gtt) !0d0
    f%b%data(2) = 0d0
    f%b%data(1) = 0d0 !aleph * f%b%data(4)
    f%rho = n
    f%p = t
    f%bmag = bmag
    f%rho2 = nnth ! this is zero if i input snnthscl=0 in the first place (checked)
	f%rhob = 0d0
	!write(*,*) 'riaf b before weights: ', f%bmag
	where(weights.eq.1d0)
		...
		f%bmag = B_arr
		f%rhob = n_arr
		!f%p = T_arr !non-thermal blob has no meaningful temperature
		!f%bmagb = B_arr
		!f%pb = T_arr
	endwhere
    call assign_metric(f%u,transpose(metric))
	...
	!write(*,*) 'Weights: ', weights
	!write(*,*) 'riaf t: ', f%p
	!write(*,*) 'riaf nnth: ', f%rho2
	!write(*,*) 'riaf total: ', f%rho+f%rho2
	!write(*,*) 'b everywhere: ', f%bmag ! zero everywhere except inside the blob (checked)
	!write(*,*) 'rhob+rho2: ', f%rhob+f%rho2

    end subroutine get_sariafblob_fluidvars
	
	subroutine get_koral3dblob_fluidvars(x0,a,f,type)
		type(four_vector), intent(in), dimension(:) :: x0
		type(fluid), intent(inout) :: f
	    real, intent(in) :: a
		! ######### variables used for blob stuff ######### 
		...
		real :: n_e, t_e, b_b
		real, dimension(size(x0)) :: n_arr, T_arr, B_arr, weights, rhob
		! ######### variables used for koral3d stuff #########
		integer, intent(in) :: type
		real(kind=8), dimension(size(x0)) :: zero,ones,uout,bout,nout,tout,rr,th
		! ######### end of koral3d stuff #########  
		u = 1.0/(x0%data(2))
		
		call koral3d_vals(x0,a,f%rho,f%p,f%b,f%u,f%bmag,f%Be, f%nnth,type)
		call blob_vals(real(x0%data(2)),real(x0%data(3)),real(x0%data(4)),n_e,t_e,b_b,n_arr,T_arr,B_arr,weights)
		!write(*,*) 'after calling blob_vals, n_e is: ', n_e
		!write(*,*) 'and n_arr is: ', n_arr
		f%weights = weights
		!write(*,*) 'in fluidvars, weights is: ', weights
		! assign u and b for koral3d part: this is already done in koral3d_vals (lines 335-346)
		! assign u and b for blob part using weights:
		where(weights.eq.1d0)
			! from get_blob_fluidvars
			g00 = -(1.0-2.0*u)
			grr = -1.0/g00
			...
			f%rhob = n_arr
		endwhere
        !write(*,*) 'f%weights: ', f%weights
	end subroutine get_koral3dblob_fluidvars
        ...
        subroutine convert_fluidvars_blob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
          ...
          !write(6,*) 'convert',size(bcgs),size(tcgs)
          !write(6,*) 'convert',size(f%rho),size(f%bmag),size(f%p)
		  !write(*,*) 'In convert, weights is: ', f%weights
          where(f%weights.eq.1d0) ! uncomment 'where' statement when using koral3d+blob
			  ncgs=f%rho; bcgs=f%bmag; tcgs=f%p; ncgsnth=f%rhob
          endwhere
          !write(*,*) 'bcgs after convert in blob: ', bcgs!, ncgs, tcgs, ncgsnth
		  !write(*,*) 'ncgsnth after convert in blob: ',ncgsnth
		  !write(*,*) 'tcgs after convert in blob: ',tcgs
        ...
        subroutine convert_fluidvars_sariafblob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
          ...
          real(kind=8), dimension(size(f%rho)),  &
          intent(inout) :: ncgs,ncgsnth,bcgs,tcgs
		  ncgs=f%rho; bcgs=f%bmag; tcgs=f%p; ncgsnth=f%rho2+f%rhob
		  !write(*,*) 'ncgsnth: ',ncgsnth
        end subroutine 
		
		subroutine convert_fluidvars_koral3dblob(f,ncgs,ncgsnth,bcgs,tcgs,nnthcgs,sp,type)
		  type (fluid), intent(in) :: f
		  integer, intent(in) :: type
		  type (source_params), intent(in) :: sp
		  real(kind=8), dimension(size(f%rho)),  &
		  intent(inout) :: ncgs,ncgsnth,bcgs,tcgs
		  real(kind=8), dimension(size(f%rho),f%nrelbin),intent(inout) :: nnthcgs
		  nnthcgs=0. !f%nnth
		  ! Convert koral3d pars first
		  call convert_fluidvars_koral(f,ncgs,ncgsnth,bcgs,tcgs,nnthcgs,sp,type)
		  ! Now convert blob pars
		  call convert_fluidvars_blob(f,ncgs,ncgsnth,bcgs,tcgs,sp)
		  !write(*,*) 'after converting fluidvars'
