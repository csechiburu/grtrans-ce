...
# now do semi-major/minor axis and orientation in terms of moments
    ...
    def get_pol_vectors(self,idex=0,pgrtrans=1,nsamp=8,trim=-1,pcut=0.05):
        ...
        #print('mx: ',np.where(np.isfinite(mx)))
        #print('my: ',np.where(np.isfinite(my)))
...
#            self.calc_spec(self.nx)
            ...
            flux_tmp=[self.spec[0,idex]*jyunit,np.sqrt(self.spec[1,idex]**2.+self.spec[2,idex]**2.)/self.spec[0,idex]*100.,
            self.spec[3,idex]/self.spec[0,idex]*100.,0.5*np.arctan2(self.spec[2,idex],self.spec[1,idex])*180./np.pi]
