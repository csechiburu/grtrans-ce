       ...
       real(8), allocatable, dimension(:) :: freqs!,mdots,m
       ...
       contains
          ...
          subroutine grtrans_main(standard,mumin,mumax,nmu,phi0,spin,&
            ...
            fbl06,fnp,ftp,frin,frout,fthin,fthout,fphiin,fphiout,fscalefac,sigcut,epcoefindx,&
			fr_c,ftheta_c,fphi_c,fn_e,ft_e,fb,fr_b,epotherargs,nepotherargs)
...
! FLUID ARGUMENTS
            ...
            real(8), intent(in) :: frspot,fr0spot,fn0spot,ftscl,frscl,fwmin,fwmax,ffmin, &
				 ...
				 fr_c,ftheta_c,fphi_c,fn_e,ft_e,fb,fr_b 
...
!            call init_pgrtrans_data(nro*nphi,NCAMS,nfreq,nmdot,nmu)
			...
			write(6,*) 'coni args:', fr_c,ftheta_c,fphi_c,fn_e,ft_e,fb,fr_b
...
!					write(*,*) 'in kwrite_raytrace_camera'     
               ivals(:,:,m)=dble(c(m)%pixvals)
!			   write(*,*) 'c(m)%pixvals', c(m)%pixvals
!               ab(:,:,m)=c(m)%pixloc
               call del_raytrace_camera(c(m))
            enddo
            call unload_fluid_model(fname)
!			write(*,*) 'after unload_fluid_model'
            deallocate(c)
            !       do i=1,nparams
