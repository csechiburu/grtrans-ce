...
!              call radtrans_integrate_formal(s,jj,KK(:,1:4),KK(:,5:7),OO)
              !write(*,*) 'calling radtrans_integrate_formal'
...
!        rI(1:nequations,1:nptsout) = intensity(1:nequations,1:nptsout);
        rnpts = nptsout
!        write(6,*) 'intensity: ', intensity !rnpts, rI(rnpts,1)
        if(isnan(intensity(1,rnpts))) then
           write(6,*) 'NaN in integrate!'
...
! let's try writing these out
           !write(*,*) 'stuff in M2',lam2,aq,sig,lam1,rhoq
		   !write(*,*) 'lam1 calculation part 1', (a2-p2)**2d0/4d0+ap**2d0
		   !write(*,*) 'lam1 calculation part 2', sqrt((a2-p2)**2d0/4d0+ap**2d0)
		   !write(*,*) 'lam1 calculation part 3', (a2-p2)/2d0
		   !write(*,*) 'lam1 calculation part 4', sqrt(sqrt((a2-p2)**2d0/4d0+ap**2d0)+(a2-p2)/2d0)
...
!        real(kind=8), dimension(4,4,size(x)), intent(inout) :: O
        ...
        integer :: k,l
        I0=0d0
		!write(*,*) 'Inside radtrans_integrate_formal'
!        identity = reshape((/1d0,0d0,0d0,0d0,0d0,1d0,0d0,0d0,0d0,0d0, &
!             1d0,0d0,0d0,0d0,0d0,1d0/),(/4,4/))
        identity(:,:)=0d0
        ...
        do k=npts-1,1,-1
		   ...
		   !if(any(isnan(at))) then
			!   write(*,*) 'nan in at'
		   !elseif(any(isnan(rhot))) then
			!   write(*,*) 'nan in rhot'
		   !elseif(isnan(dx(k))) then
			!   write(*,*) 'nan in dx(k)'
		   !elseif(any(isnan(identity))) then
			!   write(*,*) 'nan in identity'
		   !elseif(any(isnan(Ot))) then
			!   write(*,*) 'nan in Ot'
		   !elseif(any(isnan(M1))) then
			!   write(*,*) 'nan in M1'
		   !elseif(any(isnan(M2))) then
			!   write(*,*) 'nan in M2'
		   !elseif(any(isnan(M3))) then
			!   write(*,*) 'nan in M3'
		   !elseif(any(isnan(M4))) then
			!   write(*,*) 'nan in M4'
		   !endif
